...
namespace Yeast.Json
{
    public class JSON : IFromConverter<(Type, string), object, (FromJsonSettings, FromIonSettings), Exception>, IIntoConverter<object, string, (ToJsonSettings, ToIonSettings), Exception>
    {
        ...
        public static string Stringify<T>(T value, JsonStringifyMode mode = JsonStringifyMode.Compact)
        {
            ...
            return instance.TryInto(value, out string result, settings, out var exception)
                ? result
                : throw exception;
        }
        ...
        public static T Parse<T>(string text, JsonParseMode mode = JsonParseMode.Exact)
        {
            ...
            return instance.TryFrom((typeof(T), text), out object result, settings, out var exception)
                ? (T)result
                : throw exception;
        }
